...
function main()
  ...
  if data_string ~= nil then
    -- Split data
    local splited_data = split(data_string, ROVER_SEPARATOR)

    -- Remove nil values
    splited_data = removeArrayNils(splited_data)

    if getDictionarySize(splited_data) >= 1 and splited_data[1] == "CAMF" then
      output(true, 1)
      output(false, 2)
    elseif getDictionarySize(splited_data) >= 1 and splited_data[1] == "CAMR" then
      output(false, 1)
      output(true, 2)
    elseif getDictionarySize(splited_data) >= 1 and splited_data[1] == "CAMOFF" then
      output(false, 1)
      output(false, 2)
    elseif getDictionarySize(splited_data) >= 2 and splited_data[1] == "GLASSOPA" then
      output(tonumber(splited_data[2]), 3)
    elseif getDictionarySize(splited_data) >= 2 and splited_data[1] == "BACKMOTD" then
